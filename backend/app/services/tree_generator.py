...
SYSTEM_PROMPT = """你是一个专业的学习规划师。你的任务是根据用户的学习目标和当前水平，生成一棵以知识点为核心的知识图谱。
...
**绝对不允许**：零基础学生直接从应试技巧开始。必须先有扎实的基础。

## 根据用户水平调整

- **零基础**：从最基础的概念开始，level 0 应该是入门知识
- **有一些基础**：可以跳过已掌握的部分，但仍需覆盖核心基础
- **有较强基础**：可以直接进入进阶内容，但仍需确保基础扎实
...
7. **对于零基础学生，level 0-1 必须是基础知识，不能是技巧**

## 示例结构

```
level 0: 基础概念 A, 基础概念 B (无前置，最基础的入门知识)
  ↓
level 1: 进阶概念 C (依赖 A), 进阶概念 D (依赖 A, B)
  ↓
level 2: 综合应用 E (依赖 C, D), 高级概念 F (依赖 D)
  ↓
level 3: 实战技巧 G (依赖 E, F) - 只有到这里才能学技巧
```
...
async def generate_knowledge_tree(
    ...
    # 构建用户提示词，包含当前水平信息
    user_prompt = build_user_prompt(subject, goal, goal_type, deadline)
    if baseline_level:
        user_prompt += f"\n\n用户当前水平：{baseline_level}"
        if "零基础" in baseline_level or "没有基础" in baseline_level:
            user_prompt += "\n\n**重要**：用户是零基础，level 0 必须是最基础的入门知识，不能是应试技巧或高级概念。"
